...
支援模型：**DeepSeek**、**Kimi（月之暗面）**、**Qwen（通义千问）**、**文心一言**、**ChatGLM（智谱清言）**、**豆包**

1. 將 `SKILL.md` 內容複製到對話或系統提示中
2. 農曆轉換方式（無代碼執行功能時）：
   - LLM 可使用網路搜尋查詢農曆日期（如搜尋「2024年9月17日 農曆」）
   - 再套用 `SKILL.md` 中的計算公式
3. 如平台支援，可上傳 `references/` 中的參考檔案

> 💡 大多數中國大模型都內建網路搜尋功能，即使沒有代碼執行能力，也能透過搜尋準確轉換農曆日期。
